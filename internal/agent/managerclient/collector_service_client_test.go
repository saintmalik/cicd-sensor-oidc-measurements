...
func (s *fakeCollectorService) IngestLog(ctx context.Context, req *connect.Request[managerv1beta1.IngestLogRequest]) (*connect.Response[managerv1beta1.IngestLogResponse], error) {
	s.calls++
...
func TestCollectorServiceClient_SendLogBatchAddsAuthHeader(t *testing.T) {
	svc := &fakeCollectorService{
		handler: func(_ context.Context, req *connect.Request[managerv1beta1.IngestLogRequest]) (*connect.Response[managerv1beta1.IngestLogResponse], error) {
			...
			return connect.NewResponse(&managerv1beta1.IngestLogResponse{ReceivedBatches: 1}), nil
		},
	}
...
func TestCollectorServiceClient_SendBatchRetriesResourceExhausted(t *testing.T) {
	...
	if err := client.sendIngestLogBatch(context.Background(), &managerv1beta1.IngestLogBatch{CompressedJsonl: []byte{0x1f, 0x8b}}); err != nil {
...
func TestCollectorServiceClient_SendBatchDoesNotRetryInvalidArgument(t *testing.T) {
	...
	client := NewCollectorServiceClient(testCollectorServiceLogger, server.Client(), Connection{BaseURL: server.URL, Token: testOutputManagerToken})
	err := client.sendIngestLogBatch(context.Background(), &managerv1beta1.IngestLogBatch{CompressedJsonl: []byte("bad")})
	if err == nil {
...
func newFakeCollectorServer(t *testing.T, svc managerv1beta1connect.CollectorServiceHandler) *httptest.Server {
	t.Helper()
	mux := http.NewServeMux()
	path, handler := managerv1beta1connect.NewCollectorServiceHandler(svc)
